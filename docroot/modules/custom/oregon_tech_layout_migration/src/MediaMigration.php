...
namespace Drupal\oregon_tech_layout_migration;
...
class MediaMigration {
  ...
  public function setFilePath() {
    $this->mediaFilePathMapper = [
      ...
      'inline_media' => 'public://inline-images',
    ];
  }
  ...
  public function createMedia($bundle, $absoluteUrl, $attributes = []) {
    ...
    if (empty($params['username'])) {
      ...
      $fileName = str_replace('.', '-', $params['username']) . '.jpg';
    }
    $url = parse_url($absoluteUrl);
    if (empty($url['host'])) {
      $absoluteUrl = 'https://www.oit.edu/' . ltrim($absoluteUrl, '/');
    }
    else if (substr($absoluteUrl, 0, 2) == '//') {
    ...
    if ($file instanceof FileInterface) {
      ...
      if (!$media instanceof MediaInterface) {
        ...
        $path = str_replace('/sitefinity-production/images/default-source/', '', $url['path']);
        $path = str_replace('/sitefinity-production/docs/default-source/', '', $path);
        $pathParts = explode('/', $path);
        array_pop($pathParts);
        $parent = '';
        foreach ($pathParts as $group) {
          if (!empty($group)) {
            $group = ucfirst(str_replace('-', ' ', $group));

            $terms = \Drupal::entityTypeManager()
              ->getStorage('taxonomy_term')
              ->loadByProperties(['vid' => 'media_category', 'name' => $group]);
            if (!empty($terms)) {
              $term = reset($terms);
            }
            else {
              ...
              if (!empty($parent)) {
                $term->set('parent', ['target_id' => $parent]);
              }
              $term->save();
            }
            $parent = $term->id();
        ...
        if (!empty($term)) {
          $media->set('field_media_category', ['target_id' => $term->id()]);
        }
        $media->save();
      }
    }
      ...
      foreach ($files as $file) {
        ...
        if (file_exists($uri)) {
          ...
          if (!in_array($fileSize, $sizes)) {
            $sizes[] = $fileSize;
            if ($fileSize == $remoteSize || strpos($remoteUrl, '/ProfileImage.jpgp') !== FALSE) {
              $result = $file;
            }
          }
        }
        ...
        foreach ($img_tags as $img_tag) {
          ...
          if (!empty($attributes['src']) && strpos($attributes['src'], 'data:image/') === FALSE) {
            ...
            if ($this->isInternalLink($filePath)) {
              $media = $this->createMedia('inline_media', $filePath, $attributes);
              $uuid = $media instanceof MediaInterface ? $media->uuid() : '';
              if ($uuid) {
                $html = str_replace($img_tag,'<drupal-media data-align="center" data-entity-type="media" data-entity-uuid="' . $uuid . '" data-view-mode="default"></drupal-media>', $html);
              }
            }
          }
        }
  ...
  public function isInternalLink($url) {
    if (filter_var($url, FILTER_VALIDATE_URL)) {
      $url = parse_url($url);
      if (empty($url['host']) || (!empty($url['host']) && ($url['host'] == 'www.oit.edu' || $url['host'] == 'oregontechsfstatic.azureedge.net'))) {
        return TRUE;
      }
    }
    else {
