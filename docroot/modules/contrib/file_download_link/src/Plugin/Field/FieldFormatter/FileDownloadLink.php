...
class FileDownloadLink extends FileFormatterBase implements ContainerFactoryPluginInterface {
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    foreach ($this->getEntitiesToView($items, $langcode) as $delta => $file) {
      ...
      // Deal with tokens for the title and classes.
      if ($this->moduleHandler->moduleExists('token')) {
        $data = [];
        ...
        // Get the strings that may contain tokens.
        ...
        // Add the file and the entity the file is attached to
        // as token data.
        $data[$this->tokenEntityMapper->getTokenTypeForEntityType($file->getEntityTypeId())] = $file;
        $data[$entity_token_type] = $entity;
        // Finally make the calls to the token service and handle metadata.
        ...
        // The title can go right on the render array.
        // Custom classes are dealt with later.
