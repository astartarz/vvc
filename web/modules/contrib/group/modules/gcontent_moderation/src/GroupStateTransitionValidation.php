...
class GroupStateTransitionValidation extends StateTransitionValidation implements StateTransitionValidationInterface {
  ...
  public function allowedTransitions(AccountInterface $user, ContentEntityInterface $entity, array $groups) {
    ...
    return array_filter($current_state->getTransitions(), function (Transition $transition) use ($workflow, $user, $groups, $entity) {
      foreach ($groups as $group) {
        if ($group->hasPermission('use ' . $workflow->id() . ' transition ' . $transition->id() . ' for ' . $this->getPluginId($entity), $user)) {
          return TRUE;
        }
        else if ($group->hasPermission('use ' . $workflow->id() . ' transition ' . $transition->id(), $user)) {
          return TRUE;
        }
      }
    });
